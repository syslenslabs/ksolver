...
**Architecture:** In `cpsat_rust::solve`, split the anti-affinity handling: **self-pairs** (`a == b`) keep today's `x ≤ 1` per node (group_size > 1); **cross-pairs** (`a != b`) add `x_a[n] + x_b[n] ≤ 1` for every node feasible to both. In `build_pending_input`, after emitting workloads, compute cross-pairs: for each unordered pair of distinct emitted workloads in the same namespace, if one's modeled hostname selector matches **all** the other's member labels, add `(A.id, B.id)`. Correctness requires the "all members" rule so the workload-granularity constraint is exact (every A-pod conflicts with every B-pod).
...
```rust
        if !scenario.relax_required_anti_affinity {
            ...
            // Cross-pairs (a != b): at most one of {an a-replica, a b-replica} per node.
            let feasible_of: HashMap<&str, &Vec<String>> = input
                .workloads
                .iter()
                .map(|w| (w.id.as_str(), &w.feasible_nodes))
                .collect();
            for (a, b) in &input.anti_affinity_pairs {
                if a == b {
                    continue;
                }
                let (Some(fa), Some(fb)) = (feasible_of.get(a.as_str()), feasible_of.get(b.as_str()))
                else {
                    continue;
                };
                ...
                for node_name in fa.iter().filter(|n| bset.contains(n.as_str())) {
                    let xa = x_vars[&(a.clone(), node_name.clone())];
                    let xb = x_vars[&(b.clone(), node_name.clone())];
                    let expr: LinearExpr = [xa, xb].into_iter().collect();
                    model.add_le(expr, 1_i64);
                }
            }
        }
```
(`HashMap`/`HashSet`/`LinearExpr` already imported.)
