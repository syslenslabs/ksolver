...
**Preferred (soft) node affinity** (`preferredDuringScheduling`) is honored as a cost-tie-break: after the cost-optimal solve, among equally-optimal placements the scheduler prefers nodes matching more preferred weight. It runs only when the solve proves optimal and **never changes which pods are admitted or the total cost** (a two-phase pass that pins the cost optimum + admitted set, then maximizes preferred-affinity score). Preferred pod (anti-)affinity is a future slice.
