...
**Architecture:** `OptimizationInput` gains `quota_groups: Vec<QuotaGroup { workload_ids, resource, limit }>` (serde default empty). In `cpsat_rust::solve`, for each group add `Σ (per_replica_amount[w][resource] * x[w,n]) ≤ limit` over the group's workloads and their feasible nodes. Shadow's `ShadowConfig` gains `namespace_gpu_quotas` (env `KSOLVER_SHADOW_QUOTAS="ns=cap,..."`, pure-parsed). `build_pending_input` computes, per quota'd namespace, `remaining = cap - running_gpu_in_ns` (clamped ≥0) and emits a `QuotaGroup` over that namespace's pending workload ids. Broaden the not-admitted trace reason to "insufficient capacity or quota" (still contains "gang not admitted").
...
- Constraint coefficient = **per-replica** resource amount (`per_replica_scalar_requests(w).get(resource)`), matching how the solver already scales gang requests. Integer GPU amounts.
...
```
and `#[serde(default)] pub quota_groups: Vec<QuotaGroup>,` on `OptimizationInput`. Update any full `OptimizationInput { .. }` literals (cpsat tests, decision tests, bench, pending_input) — most use `..Default::default()`; the cpsat/bench ones set fields explicitly and need `quota_groups: vec![]` (compiler lists them). Build → commit.

### Task 2: Solver constraint
- [ ] In `cpsat_rust::solve`, after the anti-affinity block, add:
```rust
        for group in &input.quota_groups {
            ...
            for wid in &group.workload_ids {
                if let Some(w) = input.workloads.iter().find(|w| &w.id == wid) {
                    let per = per_replica_scalar_requests(w).get(&group.resource).copied().unwrap_or(0);
                    if per == 0 { continue; }
                    for node_name in &w.feasible_nodes {
                        expr += (per, x_vars[&(w.id.clone(), node_name.clone())]);
                    }
                }
            }
...
```
(Build an id→workload lookup once if preferred.) Feature-gated test: 2 singleton 1-GPU workloads, both feasible on a 4-GPU node, `partial_admission=true`, one `QuotaGroup{ [both ids], "nvidia.com/gpu", 1 }` → exactly one admitted; with limit 2 → both admitted. Run → commit.
...
- [ ] `build_pending_input` signature gains `quotas: &BTreeMap<String,i64>`. After emitting workloads: compute per-namespace running GPU (from `cluster.workloads` with `current_node != ""`, summing `extended_resource_requests["nvidia.com/gpu"]`); for each namespace in `quotas`, `remaining = (cap - running_gpu).max(0)`; collect emitted workload ids whose namespace == that ns; push `QuotaGroup { workload_ids, resource: "nvidia.com/gpu", limit: remaining }` (only if there are member workloads). Set `input.quota_groups`. Update `decision.rs` unresolved reason string to "...insufficient capacity or quota". Update the shadow call + all builder test call sites (pass `&Default::default()` where quotas irrelevant). Tests: a namespace quota of 1 with two 1-GPU pending singletons → 1 quota group with limit accounting for running usage; group present with correct ids/limit. Run → commit.
...
## Self-Review Notes
- Backward compatible (empty quota_groups → planner unchanged).
- Per-replica coefficient matches the solver's gang scaling; integer GPU amounts.
- Remaining quota accounts for running usage; clamped ≥0; unconfigured namespaces unconstrained.
