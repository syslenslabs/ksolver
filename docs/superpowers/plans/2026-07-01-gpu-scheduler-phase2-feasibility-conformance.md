...
- **Apples-to-apples capacity:** run `feasible_on_node` with `Options::default()` (no overcommit, no headroom) and present the simulator an EMPTY node (no other pods), so both sides see full allocatable. Document that overcommit/headroom modes are intentionally out of scope for conformance.
...
- `Options::default()` + empty node ⇒ both sides see full allocatable (documented scope).
